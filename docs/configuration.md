...
The audit events produced by this library carry the name of the application that produced them, as the `name` member
of their `application` field, and the auto-configuration insists on being able to determine it.
...
<a name="application-version"></a>
## The application version is optional

Audit events also carry the version of the application that produced them, as the `version` member of the same
`application` field, so that an entry says which build it came from. Unlike the name, the version is optional: if none
can be determined the application starts normally and the events simply leave the member out.

There are three ways of supplying it.

**1. Assign `audit.app-version` directly.** This is what a deployment does when the version comes from outside the
build, for example from a container image tag or an environment variable.
...
**2. Generate the build information.** Spring Boot's `BuildProperties` bean is created from a
`META-INF/build-info.properties` file, and the audit support picks the version up from it with no property assigned at
all. That file is **not** generated by default - enable the `build-info` goal of the Spring Boot Maven plugin:
...
**3. Let Maven filter the project version into your configuration file.** With resource filtering enabled, Maven
substitutes the POM version into the application's own configuration, so that `audit.app-version` ends up carrying it.
The Spring Boot parent POM already configures the `@` delimiter that makes this work in `application.properties` and
`application.yml`:
...
**Which one wins.** An assigned `audit.app-version` always takes precedence - including the filtered value of
alternative 3, which is an assigned property like any other. The build information is only consulted when the property
is not assigned. If neither is available, no version is included.

An application that declares an `ApplicationVersion` bean of its own keeps it, and neither the property nor the build
...
- An `ApplicationVersion` bean, resolved as described above - and no bean at all if no version can be determined.
- An `AuditEventContextResolver` bean, a `DefaultAuditEventContextResolver`, resolving the application name and
  version, the correlation ID and trace ID from the [identifier storage](tracing.html), and the principal from the
...
| :--- | :--- | :--- | :--- |
| `audit.app-version` | The version of the application, included in every audit event. If not assigned, the version from a `BuildProperties` bean is used, if available. | String | - (no version) |
...
AuditEventContextResolver auditEventContextResolver(final ApplicationName applicationName,
    final ObjectProvider<ApplicationVersion> applicationVersion) {
  return input -> new MyAuditEventContext(applicationName, applicationVersion.getIfAvailable(), input);
}
```

Note that the `ApplicationVersion` bean may be absent, so inject it as an `ObjectProvider` rather than as a required
dependency. A context reporting no version is perfectly valid.
